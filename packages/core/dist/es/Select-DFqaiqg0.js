import { defineComponent as e, inject as l, computed as a, openBlock as t, createElementBlock as o, normalizeClass as i, withModifiers as s, renderSlot as n, createVNode as u, unref as d, useSlots as r, ref as c, reactive as p, provide as h, watch as v, onMounted as f, withCtx as m, createElementVNode as b, createBlock as g, Fragment as y, renderList as V, mergeProps as _, nextTick as x, h as k } from "vue";
import { a as I } from "./Input-Bp1PNIo3.js";
import { a as M } from "./Tooltip-CdlYHpkj.js";
import { a as w } from "./Icon-ckGGeATQ.js";
import { a as S, c as O, u as B } from "./hooks-CB8aTc5l.js";
import { R as L, _ as D, d as j, w as F } from "./utils-CN-UVqEO.js";
import { B as N, C as R, h as C, D as E, b as H, s as Y, e as W, E as $, p as z, x as A, f as U, y as q, i as K, F as P, c as T } from "./vendor-Bde6p_Xx.js";
const G = Symbol("selectContext"), J = { modifiers: [{ name: "offset", options: { offset: [0, 9] } }, { name: "sameWidth", enabled: true, fn: ({ state: e2 }) => {
  ...
  const r2 = e2, c2 = l(G), p2 = a(() => N(["label", "value"], (e3) => R(C(c2, ["hightedLine", "value", "key"]), C(r2, e3)))), h2 = a(() => {
    var _a;
  ...
  function fe(e3) {
    return E(D2.options, (l3) => l3.value === e3);
  }
  const me = a(() => {
    var _a;
    return H((_a = N2.default) == null ? void 0 : _a.call(N2), (e3) => R(e3.type, X));
  ...
  h(G, { handleSelect: _e, selectStates: ne, highlightedLine: ye, renderLabel: function(e3) {
    return A(D2.renderLabel) ? D2.renderLabel(e3) : e3.label;
  } }), B(te2, (e3) => xe(e3)), v(() => D2.modelValue, () => {
    ke();
  }), l2({ blur: () => {
    xe();
  }, focus: () => {
  ...
  const Ie = c(/* @__PURE__ */ new Map()), Me = c(D2.options ?? []), we = a(() => be.value ? W(me.value, (e3) => ({ VNode: k(e3), props: q(e3.props, { disabled: true === e3.props.disabled || !K(e3.props.disabled) && !P(e3.props.disabled) }) })) : []), Se = a(() => !!D2.filterable && !Oe.value), Oe = a(() => be.value && Ie.value.size > 0 || !be.value && Y(Me.value) > 0), Be = a(() => be.value ? Ie.value.size - 1 : Y(Me.value) - 1), Le = async (e3, l3) => {
    if (!e3 || !A(e3)) return;
    let a2;
    ne.loading = true;
    try {
  ...
  }, je = a(() => D2.remote ? 300 : 100), Fe = $(() => {
    const e3 = ne.inputValue;
    ne.highlightedIndex = -1, be.value ? (async (e4) => {
      if (D2.filterable) {
        if (D2.remote && D2.remoteMethod && A(D2.remoteMethod)) return await Le(D2.remoteMethod, e4), void De(we.value);
        if (D2.filterMethod && A(D2.filterMethod)) {
          const l3 = W(D2.filterMethod(e4), "value");
          De(H(we.value, (e5) => T(l3, C(e5, ["props", "value"]))));
        } else De(H(we.value, (l3) => T(C(l3, ["props", "label"]), e4)));
      }
    })(e3) : (async (e4) => {
      D2.filterable && (D2.remote && D2.remoteMethod && A(D2.remoteMethod) ? Me.value = await Le(D2.remoteMethod, e4) : D2.filterMethod && A(D2.filterMethod) ? Me.value = D2.filterMethod(e4) : Me.value = H(D2.options, (l3) => T(l3.label, e4)));
    })(e3);
