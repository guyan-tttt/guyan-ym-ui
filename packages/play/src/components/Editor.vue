<template>
  <div class="native-editor">
    ...
    <div class="toolbar">
      ...
      <!-- 新增预设格式下拉框 -->
      <select v-model="selectFormat" @change="applyFormat($event.target.value)">
        <option value="">预设格式</option>
        <option v-for="item in styleList" :value="item.value">{{ item.label }}</option>
      </select>
    </div>
...
const props = defineProps({
  ...
  // 预设样式
  const styleList = [
    { label: "标题一",value:'h2',style: "font-size: 24px; font-weight: bold; margin: 16px 0;" },
    { label: "标题二",value:'h3',style: "font-size: 20px; font-weight: bold; margin: 16px 0;"},
    {label: "标题三", value: "h4", style: "font-size: 18px; font-weight: bold; margin: 16px 0;"},
    {label: "标题四", value: "h5", style: "font-size: 16px; font-weight: bold; margin: 16px 0;"},
    {label: "标题五", value: "h6", style: "font-size: 14px; font-weight: bold; margin: 16px 0;"},
...
const applyFormat = (format: string) => {
 ...
 const item = styleList.find(item => item.value = format)
  if(item) {
    execCommand("clearFormat")
    wrapWithTag(item.value, {style: item.style})
  }
  editor.value?.focus()
  selectFormat.value = ''
}
...
// 通用标签包裹方法
const wrapWithTag = (tagName: string, styles: Record<string, string>) => {
  const selection = window.getSelection()
  if (!selection?.rangeCount) return
  ...
  const oldNode = range.commonAncestorContainer.parentNode as HTMLElement
  ...
  const content = range.extractContents()
   ...
   oldNode.parentNode?.insertBefore(element,oldNode)
