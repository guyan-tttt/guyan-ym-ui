...
调用 `YmMessageBox.alert` 方法以打开 alert 框。
...
调用 `YmMessageBox.confirm` 方法以打开 confirm 框。
...
调用 `YmMessageBox.prompt` 方法以打开 prompt 框。
...
可以通过配置 `YmMessageBox` 的参数来实现一些个性化需求（options 参数见下文）。
...
`YmMessageBox` 提供了全局方法 `YmMessageBox.alert`、`YmMessageBox.confirm`、`YmMessageBox.prompt`，用于在需要时弹出消息弹框。
